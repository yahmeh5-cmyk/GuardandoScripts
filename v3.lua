...
================================================================================
  ...
  COMO INSTALAR
    ...
    3. Apague o conteÃºdo e cole ESTE arquivo inteiro
    4. Play. O painel abre com RIGHT SHIFT.

  ATALHOS PADRÃƒO (todos remapeÃ¡veis na aba UTILITÃRIOS)
    RightShift .... abrir / fechar painel
    RightControl .. liga/desliga o ESP (master)
    F ............. voar
    G ............. noclip
    T ............. teleporte no clique do mouse
    K ............. crosshair
    End ........... PANIC: destrÃ³i tudo e restaura o jogo

  ESTRUTURA
    01 serviÃ§os e helpers      06 engine de ESP
    02 tema                    07 features (movimento/visual/mundo/players)
    03 estado (settings)       08 construÃ§Ã£o da interface
    04 biblioteca de UI        09 keybinds + init
    05 notificaÃ§Ãµes

  OBS: tudo Ã© local (sÃ³ vocÃª vÃª). Nada aqui altera o servidor.
================================================================================
]]
...
-- registro de conexÃµes para limpeza total no PANIC
local Conns = {}
...
local FONTS = {
	...
	SourceSansBold = Enum.Font.SourceSansBold, Fantasy = Enum.Font.Fantasy,
	Arcade = Enum.Font.Arcade,
}

--==============================================================================
-- 02 Â· TEMA
-- superfÃ­cies escuras levemente puxadas pro tom do accent (nada de cinza morto)
--==============================================================================
...
--==============================================================================
-- 03 Â· ESTADO / SETTINGS
-- Tudo mora aqui. Cada controle da UI aponta pra uma chave via "flag".
--==============================================================================
...
local EspHolder = new("Frame", {
	Name = "Holder", BackgroundTransparency = 1, Size = UDim2.fromScale(1, 1), Parent = EspGui,
})
...
local function label(parent, text, props)
	local p = {
		Text = text, Font = T.Font, TextSize = 13, TextColor3 = T.Text,
...
-- drag genÃ©rico: entrega posiÃ§Ã£o absoluta enquanto o botÃ£o estÃ¡ pressionado
local function dragHook(target, fn)
	target.InputBegan:Connect(function(i)
		...
		ec = UserInputService.InputEnded:Connect(function(i3)
			if i3.UserInputType == i.UserInputType then
...
------------------------------------------------------------------ janela ------
local Win = new("Frame", {
	Name = "Window", Size = UDim2.fromOffset(790, 500),
	Position = UDim2.new(0.5, -395, 0.5, -250),
...
local Side = new("Frame", {
	Size = UDim2.new(0, 186, 1, 0), BackgroundColor3 = T.Bg2, BorderSizePixel = 0, Parent = Win,
})
corner(14, Side)
...
new("Frame", { Size = UDim2.fromOffset(6, 26), Position = UDim2.fromOffset(20, 26),
	BackgroundColor3 = T.Accent, BorderSizePixel = 0, Parent = brand }, { new("UICorner", { CornerRadius = UDim.new(1, 0) }) })
...
local TabBar = new("ScrollingFrame", {
	Size = UDim2.new(1, -24, 1, -78 - 62), Position = UDim2.fromOffset(12, 78),
...
local footPing = label(SideFoot, "-- ms  Â·  --", { Position = UDim2.fromOffset(12, 26), Size = UDim2.new(1, -20, 0, 12),
	Font = Enum.Font.Code, TextSize = 10, TextColor3 = T.Dim })

-- cabeÃ§alho
local Head = new("Frame", {
	Size = UDim2.new(1, -186, 0, 76), Position = UDim2.fromOffset(186, 0),
...
new("Frame", { Size = UDim2.new(1, -186 - 44, 0, 1), Position = UDim2.fromOffset(186 + 22, 76),
	BackgroundColor3 = T.Stroke2, BorderSizePixel = 0, Parent = Win })

local Pages = new("Frame", {
	Size = UDim2.new(1, -186 - 24, 1, -92), Position = UDim2.fromOffset(186 + 12, 86),
	BackgroundTransparency = 1, Parent = Win,
})

-- arrastar a janela pelo cabeÃ§alho
do
	local dragging, startPos, startAbs = false, nil, nil
	Head.InputBegan:Connect(function(i)
		if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
			dragging, startPos, startAbs = true, Win.Position, i.Position
		end
	end)
	bind(UserInputService.InputChanged, function(i)
		if not dragging then return end
		if i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch then
			local d = i.Position - startAbs
...
do
	local grip = new("TextButton", {
		Size = UDim2.fromOffset(16, 16), Position = UDim2.new(1, -18, 1, -18), Text = "",
		BackgroundTransparency = 1, Parent = Win,
	})
	for i = 1, 3 do
		new("Frame", { Size = UDim2.fromOffset(3, 3), Position = UDim2.fromOffset(12 - (i - 1) * 5, 12),
...
local NotifHolder = new("Frame", {
	Size = UDim2.fromOffset(300, 400), Position = UDim2.new(1, -316, 1, -416),
...
local function notify(title, body, dur, kind)
	...
	local card = new("Frame", {
		Size = UDim2.new(1, 0, 0, body and 54 or 38), BackgroundColor3 = T.Card,
		BorderSizePixel = 0, LayoutOrder = notifId, Parent = NotifHolder,
		BackgroundTransparency = 1,
	})
	corner(10, card)
	local st = stroke(T.Stroke, 1, card, 1)
	new("Frame", { Size = UDim2.fromOffset(4, 4), Position = UDim2.fromOffset(14, body and 17 or 17),
...
function Section:_row(h)
	...
	vlist(6, row)
	local head = new("Frame", {
		Size = UDim2.new(1, 0, 0, h or 24), BackgroundTransparency = 1, LayoutOrder = 1, Parent = row,
	})
	return row, head
end
...
local function setFlag(o, v)
	if o.flag then S[o.flag] = v end
	if o.callback then
		local ok, err = pcall(o.callback, v)
		if not ok then notify("Erro em " .. tostring(o.text), tostring(err), 5, "bad") end
	end
end
...
function Section:Toggle(o)
	...
	local track = new("Frame", {
		Size = UDim2.fromOffset(34, 18), Position = UDim2.new(1, -34, 0.5, -9),
		BackgroundColor3 = T.Bg, BorderSizePixel = 0, Parent = head,
	})
	corner(9, track)
	local ts = stroke(T.Stroke, 1, track)
	local knob = new("Frame", {
		Size = UDim2.fromOffset(12, 12), Position = UDim2.fromOffset(3, 3),
	...
	local function apply(v, fire)
		state = v
		tween(knob, 0.16, { Position = UDim2.fromOffset(v and 19 or 3, 3), BackgroundColor3 = v and T.Bg or T.Dim })
		tween(track, 0.16, { BackgroundColor3 = v and T.Accent or T.Bg })
		tween(ts, 0.16, { Color = v and T.Accent or T.Stroke })
		if o.flag then S[o.flag] = v end
		if fire ~= false and o.callback then
			local ok, err = pcall(o.callback, v)
			if not ok then notify("Erro: " .. o.text, tostring(err), 5, "bad") end
		end
	end
	...
	return { Set = apply, Get = function() return state end, Row = row }
end
...
function Section:Slider(o)
	...
	local cur = initial(o) or mn
	local function apply(v, fire)
		v = math.clamp(round(v, dec), mn, mx)
		cur = v
	...
	apply(cur, false)
	if o.flag then Lib.Flags[o.flag] = function(v) apply(tonumber(v) or mn, true) end end
...
function Section:Button(o)
	...
	local b = new("TextButton", {
		Size = UDim2.fromScale(1, 1), BackgroundColor3 = o.danger and Color3.fromRGB(58, 32, 38) or T.Card2,
		Text = o.text, Font = T.FontM, TextSize = 12.5,
	...
	b.MouseLeave:Connect(function() tween(b, 0.12, { BackgroundColor3 = o.danger and Color3.fromRGB(58, 32, 38) or T.Card2 }) end)
	b.MouseButton1Click:Connect(function()
		tween(b, 0.08, { BackgroundColor3 = T.Accent })
		task.delay(0.1, function() tween(b, 0.14, { BackgroundColor3 = o.danger and Color3.fromRGB(58, 32, 38) or T.Card2 }) end)
		if o.callback then
...
function Section:Dropdown(o)
	local row, head = self:_row(o.text ~= "" and 44 or 26)
	if o.text ~= "" then
		label(head, o.text, { Size = UDim2.new(1, 0, 0, 16), TextSize = 12.5 })
	end
	local btn = new("TextButton", {
		Size = UDim2.new(1, 0, 0, 26), Position = UDim2.new(0, 0, 0, o.text ~= "" and 18 or 0),
	...
	local listF = new("ScrollingFrame", {
		Size = UDim2.new(1, 0, 0, 100), BackgroundColor3 = T.Bg, BorderSizePixel = 0,
		Visible = false, LayoutOrder = 2, ScrollBarThickness = 3, CanvasSize = UDim2.new(),
		AutomaticCanvasSize = Enum.AutomaticSize.Y, ScrollBarImageColor3 = T.Stroke, Parent = row,
	})
	corner(8, listF); stroke(T.Stroke2, 1, listF)
	padding(4, 4, 4, 4, listF)
	vlist(2, listF)

	local options, chosen = o.options or {}, nil
	...
	local function text()
		...
		return tostring(chosen or "-")
	end

	local rebuild
	local function fire()
		sel.Text = text()
		sel.TextColor3 = (multi and #chosen > 0) or (not multi and chosen) and T.Text or T.Sub
	...
	rebuild = function()
		...
		for idx, opt in ipairs(options) do
			local on = multi and table.find(chosen, opt) ~= nil or (not multi and chosen == opt)
			local ob = new("TextButton", {
				Size = UDim2.new(1, 0, 0, 24), BackgroundColor3 = on and T.Card2 or T.Bg,
				BackgroundTransparency = on and 0 or 1, Text = "", AutoButtonColor = false,
				LayoutOrder = idx, BorderSizePixel = 0, Parent = listF,
			})
			corner(6, ob)
			...
			if on then
				label(ob, "â€¢", { Position = UDim2.new(1, -18, 0, 0), Size = UDim2.fromOffset(12, 24),
					TextColor3 = T.Accent, Font = T.FontB })
			end
			ob.MouseButton1Click:Connect(function()
				if multi then
					local i = table.find(chosen, opt)
					if i then table.remove(chosen, i) else table.insert(chosen, opt) end
				else
					chosen = opt
					listF.Visible = false
					arrow.Text = "â–¾"
	...
	function api.Set(v) chosen = v; rebuild(); fire() end
...
function Section:TextBox(o)
	local row, head = self:_row(o.text ~= "" and 44 or 26)
	if o.text ~= "" then label(head, o.text, { Size = UDim2.new(1, 0, 0, 16), TextSize = 12.5 }) end
	local box = new("Frame", {
		Size = UDim2.new(1, 0, 0, 26), Position = UDim2.new(0, 0, 0, o.text ~= "" and 18 or 0),
		BackgroundColor3 = T.Bg, BorderSizePixel = 0, Parent = head,
	})
	...
	local tb = new("TextBox", {
		...
		Text = tostring(initial(o) or ""), PlaceholderText = o.placeholder or "",
		PlaceholderColor3 = T.Dim, Font = T.Font, TextSize = 12, TextColor3 = T.Text,
	...
	return { Box = tb, Get = function() return tb.Text end, Set = function(v) tb.Text = tostring(v) end }
end
...
function Section:Keybind(o)
	...
	local b = new("TextButton", {
		Size = UDim2.fromOffset(86, 22), Position = UDim2.new(1, -86, 0.5, -11),
		BackgroundColor3 = T.Bg, Text = tostring(initial(o) or "-"), Font = Enum.Font.Code,
		TextSize = 11, TextColor3 = T.Sub, AutoButtonColor = false, BorderSizePixel = 0, Parent = head,
	})
	...
	b.MouseButton1Click:Connect(function()
		...
		capturing = function(keyName)
			...
			if o.callback then pcall(o.callback, keyName) end
		end
	end)
...
function Section:ColorPicker(o)
	...
	local sw = new("TextButton", {
		Size = UDim2.fromOffset(40, 18), Position = UDim2.new(1, -40, 0.5, -9),
	...
	local panel = new("Frame", {
		Size = UDim2.new(1, 0, 0, 118), BackgroundColor3 = T.Bg, BorderSizePixel = 0,
		Visible = false, LayoutOrder = 2, Parent = row,
	})
	corner(8, panel); stroke(T.Stroke2, 1, panel); padding(8, 8, 8, 8, panel)

	local sv = new("Frame", {
		Size = UDim2.new(1, -34, 1, -22), BackgroundColor3 = Color3.new(1, 0, 0), BorderSizePixel = 0, Parent = panel,
	})
	corner(6, sv)
	...
	corner(6, black)
	new("UIGradient", {
		Rotation = 90, Color = ColorSequence.new(Color3.new(0, 0, 0)),
		Transparency = NumberSequence.new({ NumberSequenceKeypoint.new(0, 1), NumberSequenceKeypoint.new(1, 0) }),
		Parent = black,
	})
	local cursor = new("Frame", {
		Size = UDim2.fromOffset(9, 9), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundTransparency = 1,
		Parent = sv, ZIndex = 5,
	})
	corner(5, cursor); stroke(Color3.new(1, 1, 1), 2, cursor)

	local hue = new("Frame", {
		Size = UDim2.fromOffset(20, 96), Position = UDim2.new(1, -20, 0, 0),
		BorderSizePixel = 0, BackgroundColor3 = Color3.new(1, 1, 1), Parent = panel,
	})
	corner(6, hue)
	new("UIGradient", {
		Rotation = 90,
	...
	local hcur = new("Frame", {
		Size = UDim2.new(1, 4, 0, 3), AnchorPoint = Vector2.new(0.5, 0.5), Position = UDim2.new(0.5, 0, 0, 0),
		BackgroundColor3 = Color3.new(1, 1, 1), BorderSizePixel = 0, Parent = hue, ZIndex = 5,
	})
	local hexBox = new("TextBox", {
		Size = UDim2.new(1, -34, 0, 16), Position = UDim2.new(0, 0, 1, -16), BackgroundTransparency = 1,
		Font = Enum.Font.Code, TextSize = 11, TextColor3 = T.Dim, Text = "", ClearTextOnFocus = false,
		TextXAlignment = Enum.TextXAlignment.Left, Parent = panel,
	})

	local c = initial(o) or Color3.new(1, 1, 1)
...
function Section:Label(text, color)
	...
	local l = label(head, text, {
		Size = UDim2.new(1, 0, 0, 0), AutomaticSize = Enum.AutomaticSize.Y,
		TextSize = 11.5, TextColor3 = color or T.Sub, TextWrapped = true,
		TextYAlignment = Enum.TextYAlignment.Top,
	})
	return { Label = l, Set = function(v) l.Text = v end }
end

function Section:Divider()
	local row, head = self:_row(1)
	head.BackgroundColor3 = T.Stroke2
	head.BackgroundTransparency = 0
	head.BorderSizePixel = 0
end

-- console rolÃ¡vel reutilizÃ¡vel (chat log, explorer, remotes)
function Section:Console(height)
	local row, head = self:_row(height or 150)
	local sf = new("ScrollingFrame", {
		Size = UDim2.fromScale(1, 1), BackgroundColor3 = T.Bg, BorderSizePixel = 0,
	...
	function api.Add(text, color, cb)
		...
		if cb then
			...
			host.MouseButton1Click:Connect(cb)
		else
			host = new("Frame", { Size = UDim2.new(1, 0, 0, 0), AutomaticSize = Enum.AutomaticSize.Y,
				BackgroundTransparency = 1, LayoutOrder = n, Parent = sf })
		end
		label(host, text, {
			Size = UDim2.new(1, 0, 0, 0), AutomaticSize = Enum.AutomaticSize.Y, Font = Enum.Font.Code,
			TextSize = 11.5, TextColor3 = color or T.Sub, TextWrapped = true,
			TextYAlignment = Enum.TextYAlignment.Top,
		})
		task.defer(function() sf.CanvasPosition = Vector2.new(0, math.max(sf.AbsoluteCanvasSize.Y, 0)) end)
		return host
	end
	function api.Clear()
		n = 0
...
function Lib:Tab(name, desc)
	...
	local btn = new("TextButton", {
		Size = UDim2.new(1, 0, 0, 32), BackgroundColor3 = T.Card, BackgroundTransparency = 1,
		Text = "", AutoButtonColor = false, LayoutOrder = order, BorderSizePixel = 0, Parent = TabBar,
	})
	corner(8, btn)
	local bar = new("Frame", {
		Size = UDim2.fromOffset(2, 0), Position = UDim2.fromOffset(0, 16), AnchorPoint = Vector2.new(0, 0.5),
	...
	local page = new("ScrollingFrame", {
		...
		ScrollBarThickness = 4, ScrollBarImageColor3 = T.Stroke, CanvasSize = UDim2.new(),
		AutomaticCanvasSize = Enum.AutomaticSize.Y, Parent = Pages,
	})
	padding(0, 8, 20, 0, page)
	hlist(12, page)
	local cols = {}
	for i = 1, 2 do
		cols[i] = new("Frame", {
			Size = UDim2.new(0.5, -6, 0, 0), AutomaticSize = Enum.AutomaticSize.Y,
			BackgroundTransparency = 1, LayoutOrder = i, Parent = page,
		})
		vlist(12, cols[i])
	end

	local tab = { name = name, page = page, cols = cols, btn = btn, _sec = 0 }
	...
	function tab:Section(title, side)
		...
		vlist(9, f)
		new("TextLabel", {
			Text = string.upper(title), Font = T.FontB, TextSize = 10.5, TextColor3 = T.Dim,
			TextXAlignment = Enum.TextXAlignment.Left, BackgroundTransparency = 1,
			Size = UDim2.new(1, 0, 0, 13), LayoutOrder = 0, Parent = f,
		})
		local sec = setmetatable({ frame = f, _n = 0 }, Section)
		return sec
	end
	...
	local function select()
		...
		page.Visible = true
		tween(btn, 0.16, { BackgroundTransparency = 0, BackgroundColor3 = T.Card })
	...
	btn.MouseEnter:Connect(function()
		if not page.Visible then tween(btn, 0.12, { BackgroundTransparency = 0.6, BackgroundColor3 = T.Card }) end
	end)
...
local function mkLine(parent, z)
	return new("Frame", {
		...
		Size = UDim2.fromOffset(1, 1), Visible = false, ZIndex = z or 2, Parent = parent,
	})
end
...
local function mkText(parent, size, align)
	return new("TextLabel", {
		BackgroundTransparency = 1, Font = T.FontB, TextSize = size or 13, TextColor3 = T.Text,
...
local function createDrawing()
	...
	d.cor = {}
	for i = 1, 8 do d.cor[i] = mkLine(d.holder) end
	d.b3 = {}
	for i = 1, 12 do d.b3[i] = mkLine(d.holder) end
	d.bone = {}
	for i = 1, 16 do d.bone[i] = mkLine(d.holder) end
	...
	d.hpFill = new("Frame", { BackgroundColor3 = T.Good, BorderSizePixel = 0, AnchorPoint = Vector2.new(0, 1),
	...
	d.hpTxt = mkText(d.holder, 11, Enum.TextXAlignment.Right)
...
-- monta a lista de alvos (roda a cada S.ESP_Refresh segundos)
function ESP:Refresh()
		...
		for _, o in ipairs(Workspace:GetDescendants()) do
			if o:IsA("Humanoid") then
				...
				if m and m:IsA("Model") and not Players:GetPlayerFromCharacter(m) then
					...
					if r then
						table.insert(list, { model = m, humanoid = o, type = "NPC", name = m.Name })
					end
				end
			end
		end
		...
		if #classes > 0 or #words > 0 then
			...
			for _, o in ipairs(Workspace:GetDescendants()) do
				...
				if (hitClass or hitWord) and not o:IsDescendantOf(HLFolder) then
					...
					if not part and not model then
						part = o:FindFirstChildWhichIsA("BasePart", true)
					end
					if part or model then
						if not (chr() and o:IsDescendantOf(chr())) then
							count += 1
	...
	-- limpa desenhos de alvos que sumiram
	local keep = {}
...
local function checkVisible(t, pos, camPos)
	...
	if chr() then table.insert(ignore, chr()) end
...
local function statusLines(t, dist)
	local lines = {}
	local h = t.humanoid
	local p = t.player
	if S.ST_Health and h then
		if S.ST_MaxHealth then
			table.insert(lines, string.format("HP %d/%d", math.floor(h.Health), math.floor(h.MaxHealth)))
		else
			table.insert(lines, "HP " .. math.floor(h.Health))
		end
	elseif S.ST_MaxHealth and h then
		table.insert(lines, "MAX " .. math.floor(h.MaxHealth))
	end
	...
	if S.ST_Leaderstats and p then
		...
		if ls then
			for _, v in ipairs(ls:GetChildren()) do
				if v:IsA("ValueBase") then table.insert(lines, v.Name .. ": " .. tostring(v.Value)) end
			end
		end
	end
	if S.ST_Attributes and t.model then
		for k, v in pairs(t.model:GetAttributes()) do
			table.insert(lines, k .. " = " .. tostring(v))
		end
	end
	if S.ST_Velocity then
		local r = t.model and (t.model:FindFirstChild("HumanoidRootPart") or (t.model:IsA("Model") and t.model.PrimaryPart)) or t.part
		if r and r:IsA("BasePart") then
			table.insert(lines, "vel " .. round(r.AssemblyLinearVelocity.Magnitude, 1))
		end
	end
	if S.ST_Position then
		local r = t.part or (t.model and (t.model:FindFirstChild("HumanoidRootPart") or (t.model:IsA("Model") and t.model.PrimaryPart)))
		if r and r:IsA("BasePart") then
			local pp = r.Position
			table.insert(lines, string.format("%d, %d, %d", pp.X, pp.Y, pp.Z))
		end
	end
	return lines
end
...
local function renderTarget(t, d, camPos, vp)
	...
	if S.ESP_Arrows and not onScreen then
		...
		d.arrow.Position = UDim2.fromOffset(center.X + u.X * (math.min(vp.X, vp.Y) * 0.33), center.Y + u.Y * (math.min(vp.X, vp.Y) * 0.33))
	...
	if sp.Z <= 0 then
		for _, e in ipairs({ d.box, d.fill, d.hpBg, d.dot, d.name, d.dist, d.info, d.hpTxt, d.tracer, d.look }) do e.Visible = false end
		for i = 1, 8 do d.cor[i].Visible = false end
		for i = 1, 12 do d.b3[i].Visible = false end
		for i = 1, 16 do d.bone[i].Visible = false end
	...
	-- caixa a partir da bounding box real
	local cf, size
	...
	for i = 1, 8 do
		...
		local s2, _ = Camera:WorldToViewportPoint(wp)
		if s2.Z <= 0 then bad = true break end
		pts[i] = Vector2.new(s2.X, s2.Y)
	...
	if bad then
		minX, minY, maxX, maxY = sp.X - 20, sp.Y - 30, sp.X + 20, sp.Y + 30
	end
	...
	local isFull = S.ESP_BoxType == "Completa"
	local is3D = S.ESP_BoxType == "3D"
	d.box.Visible = wantBox and isFull
	...
	if wantBox and S.ESP_BoxType == "Cantos" then
		...
		for i = 1, 8 do setLine(d.cor[i], L[i][1], L[i][2], thick, col) end
	end
	for i = 1, 12 do d.b3[i].Visible = false end
	if wantBox and is3D and not bad then
		for i, e in ipairs(EDGES) do setLine(d.b3[i], pts[e[1]], pts[e[2]], thick, col) end
	end
	...
	local bottom = {}
	if S.ESP_Distance then table.insert(bottom, math.floor(dist) .. "m") end
	if S.ESP_HealthText and t.humanoid then
		table.insert(bottom, math.floor(t.humanoid.Health) .. "/" .. math.floor(t.humanoid.MaxHealth) .. " hp")
	end
	...
	---------------------------------------------------------------- status ----
	local lines = t.type == "Object" and {} or statusLines(t, dist)
	...
	------------------------------------------------------------ barra de hp ---
	local wantHp = S.ESP_HealthBar and t.humanoid
	d.hpBg.Visible = wantHp == true and wantHp ~= nil
	if wantHp then
		...
		for _, pair in ipairs(set) do
			...
			if a and b and a:IsA("BasePart") and b:IsA("BasePart") then
				...
				if sa.Z > 0 and sb.Z > 0 then
					i += 1
					if i <= 16 then
						setLine(d.bone[i], Vector2.new(sa.X, sa.Y), Vector2.new(sb.X, sb.Y), math.max(thick, 1), col)
					end
				end
			end
		end
	...
	if S.ESP_LookVector and t.type ~= "Object" then
		...
		if origin and origin:IsA("BasePart") then
			...
			if a.Z > 0 and b.Z > 0 then
				setLine(d.look, Vector2.new(a.X, a.Y), Vector2.new(b.X, b.Y), thick, col)
			end
		end
	end
...
local function espStep()
	...
	for _, t in ipairs(ESP.Targets) do
		...
		if m and m.Parent then
			...
			if not ok then
				espErrors += 1
				if espErrors == 1 then notify("ESP: erro isolado", tostring(err), 6, "warn") end
			end
		end
	end
end
...
local ORIG = {
	...
	ZoomMax = LocalPlayer.CameraMaxZoomDistance, ZoomMin = LocalPlayer.CameraMinZoomDistance,
}
...
local function charStep()
	...
	if S.JumpOn then
		if h.UseJumpPower then
			...
			if h.JumpHeight ~= S.JumpValue then h.JumpHeight = S.JumpValue end
		end
	end
...
local function applyLighting()
	...
	if S.ClockTimeOn then Lighting.ClockTime = S.ClockTime else Lighting.ClockTime = ORIG.ClockTime end
...
local function lowGFX(on)
	if on then
		...
		notify("Low GFX", #removed .. " texturas removidas localmente", 3, "good")
	else
...
local function noParticles(on)
	for _, o in ipairs(Workspace:GetDescendants()) do
		if o:IsA("ParticleEmitter") or o:IsA("Trail") or o:IsA("Smoke") or o:IsA("Fire") or o:IsA("Sparkles") or o:IsA("Beam") then
			o.Enabled = not on
		end
	end
end
...
local function hideNames(on)
	for _, p in ipairs(Players:GetPlayers()) do
		local c = p.Character
		local h = c and c:FindFirstChildOfClass("Humanoid")
		if h then h.DisplayDistanceType = on and Enum.HumanoidDisplayDistanceType.None or Enum.HumanoidDisplayDistanceType.Viewer end
	end
end

------------------------------------------------------------------ crosshair ---
local CrossHolder = new("Frame", {
	Size = UDim2.fromOffset(60, 60), AnchorPoint = Vector2.new(0.5, 0.5),
	Position = UDim2.fromScale(0.5, 0.5), BackgroundTransparency = 1, Visible = false, Parent = OverlayGui,
})
local crossParts = {}
for i = 1, 4 do
	crossParts[i] = new("Frame", {
		BackgroundColor3 = T.Accent, BorderSizePixel = 0, AnchorPoint = Vector2.new(0.5, 0.5), Parent = CrossHolder,
	})
end
crossParts[5] = new("Frame", {
	Size = UDim2.fromOffset(2, 2), AnchorPoint = Vector2.new(0.5, 0.5), Position = UDim2.fromScale(0.5, 0.5),
	BackgroundColor3 = T.Accent, BorderSizePixel = 0, Parent = CrossHolder,
})
local function applyCrosshair()
	...
	for i = 1, 4 do
		local d = dirs[i]
		local vert = d[1] == 0
		crossParts[i].Size = vert and UDim2.fromOffset(2, L) or UDim2.fromOffset(L, 2)
		crossParts[i].Position = UDim2.new(0.5, d[1] * (gap + L * 0.5), 0.5, d[2] * (gap + L * 0.5))
		crossParts[i].BackgroundColor3 = S.CrosshairColor
	end
...
local function setFreecam(on)
	...
	if on then
		...
		notify("Freecam", "WASD + EspaÃ§o/Ctrl. Q/E gira.", 4, "good")
	else
...
--------------------------------------------------------------------- ABA ESP --
local TabESP = Lib:Tab("ESP", "VisualizaÃ§Ã£o de players, NPCs e objetos atravÃ©s de paredes")
...
sVis:Dropdown({ text = "Origem do tracer", flag = "ESP_TracerFrom", options = { "Base da tela", "Centro", "Topo", "Mouse" } })
...
sObj:Dropdown({ text = "Classes", flag = "OBJ_Classes", multi = true, callback = function() ESP:Refresh() end,
	options = { "Tool", "Part", "MeshPart", "Model", "SpawnLocation", "ProximityPrompt", "ClickDetector",
		"VehicleSeat", "Seat", "Sound", "Humanoid", "TouchTransmitter", "Attachment" } })
sObj:TextBox({ text = "Palavras-chave (separadas por vÃ­rgula)", flag = "OBJ_Keywords",
...
sCol:Dropdown({ text = "Fonte", flag = "ESP_Font",
	options = { "Gotham", "GothamMedium", "GothamBold", "GothamBlack", "Code", "SourceSans", "SourceSansBold", "Fantasy", "Arcade" } })
...
sWp:Button({ text = "Apagar waypoint selecionado", danger = true, callback = function()
	...
	for i, w in ipairs(S.Waypoints) do
		if w.name == n then table.remove(S.Waypoints, i) break end
	end
...
local upLbl = sSrv:Label("Uptime: 0s")
...
local function drawExplorer()
	...
	for i, c in ipairs(kids) do
		if i > 120 then expConsole.Add("... +" .. (#kids - 120) .. " itens", T.Dim) break end
...
sExp:Button({ text = "Recarregar / ir para Workspace", callback = function()
...
sRem:Button({ text = "Listar remotes do jogo", callback = function()
	...
	for _, o in ipairs(game:GetDescendants()) do
		if o:IsA("RemoteEvent") or o:IsA("RemoteFunction") then
			...
			local o2 = o
			remConsole.Add(o.ClassName:sub(7, 12) .. " Â· " .. o:GetFullName(), o:IsA("RemoteEvent") and T.Good or T.Warn, function()
				remSelected = o2
				notify("Remote selecionado", o2.Name, 2, "good")
			end)
		end
	end
...
sRem:Button({ text = "Disparar remote selecionado", danger = true, callback = function()
	...
	for a in string.gmatch(remArgs.Get(), "[^,]+") do
		...
		elseif a ~= "" then table.insert(args, a) end
	end
	local ok, err = pcall(function()
		if remSelected:IsA("RemoteEvent") then remSelected:FireServer(unpack(args))
		else
			local r = remSelected:InvokeServer(unpack(args))
			notify("Retorno", tostring(r), 4, "good")
		end
	end)
...
local function drawPlayers()
	...
	for _, p in ipairs(Players:GetPlayers()) do
		...
		if r and pr then dist = math.floor((r.Position - pr.Position).Magnitude) .. "m" end
		local line = string.format("%s%s  Â·  %s hp  Â·  %s",
			p.Name, p == LocalPlayer and " (vocÃª)" or "",
			h and tostring(math.floor(h.Health)) or "?", dist)
		plConsole.Add(line, p == LocalPlayer and T.Accent or (h and h.Health > 0 and T.Sub or T.Dim), function()
			notify(p.Name, string.format("id %d Â· %d dias Â· time %s", p.UserId, p.AccountAge, p.Team and p.Team.Name or "none"), 5, "good")
...
local function logChat(name, msg)
	logConsole.Add("[" .. os.date("%H:%M") .. "] " .. name .. ": " .. msg, T.Sub)
end
local function hookChat(p)
	bind(p.Chatted, function(msg) logChat(p.Name, msg) end)
end
for _, p in ipairs(Players:GetPlayers()) do hookChat(p) end
bind(Players.PlayerAdded, function(p)
	hookChat(p)
	logConsole.Add("[+] " .. p.Name .. " entrou", T.Good)
	refreshPlayers(); drawPlayers()
end)
bind(Players.PlayerRemoving, function(p)
	logConsole.Add("[-] " .. p.Name .. " saiu", T.Bad)
	task.defer(function() refreshPlayers(); drawPlayers() end)
end)

------------------------------------------------------------- ABA UTILITÃRIOS --
local TabUtil = Lib:Tab("UtilitÃ¡rios", "Atalhos, aparÃªncia do painel e configuraÃ§Ãµes")
...
sKeys:Keybind({ text = "PANIC (destruir tudo)", flag = "KB_Panic" })

local sUI = TabUtil:Section("AparÃªncia do painel", 1)
sUI:Slider({ text = "Escala da interface", flag = "UIScale", min = 0.6, max = 1.6, decimals = 2, callback = function(v)
	WinScale.Scale = v
end })
sUI:Slider({ text = "TransparÃªncia", flag = "UITransparency", min = 0, max = 0.6, decimals = 2, callback = function(v)
	Win.BackgroundTransparency = v
	Side.BackgroundTransparency = v
end })
sUI:ColorPicker({ text = "Cor de destaque", default = T.Accent, callback = function(c)
	T.Accent = c
	for _, o in ipairs(PanelGui:GetDescendants()) do
		if (o:IsA("Frame") or o:IsA("UIStroke")) and o.Name == "AccentPart" then end
	end
	notify("Tema", "novos elementos usarÃ£o essa cor", 3)
end })
...
local function serialize()
	...
	for k, v in pairs(S) do
		...
		elseif type(v) == "table" then out[k] = v
		else out[k] = v end
	end
...
sCfg:Button({ text = "Salvar configuraÃ§Ã£o", callback = function()
	...
	if saved then
		notify("Config", "salvo em nexus_admin_config.json", 3, "good")
	elseif clip(json) then
		notify("Config", "JSON copiado pro clipboard", 3, "good")
	else
		notify("Config", "sem acesso a arquivo/clipboard, use o campo abaixo", 4, "warn")
	end
end })
sCfg:Button({ text = "Carregar configuraÃ§Ã£o salva", callback = function()
	...
	notify("Config", "nada salvo em arquivo, cole o JSON abaixo", 4, "warn")
end })
...
sHelp:Label("Nexus Admin Suite v" .. VERSION .. "\n\nTudo aqui roda no cliente: nada Ã© enviado ao servidor exceto se vocÃª usar o disparador de remotes.\n\nESP: 28 opÃ§Ãµes + 15 linhas de status.\nExtras: 40+ funcionalidades entre movimento, cÃ¢mera, mundo, players e utilitÃ¡rios.")
sHelp:Divider()
sHelp:Label("Dica: ESP de objetos usa classe OU palavra-chave. Ex: palavra 'chest' acha qualquer coisa com chest no nome.", T.Dim)
sHelp:Button({ text = "NotificaÃ§Ã£o de teste", callback = function()
	notify("Funcionando", "se vocÃª viu isso, tÃ¡ tudo certo", 3, "good")
end })
sHelp:Button({ text = "PANIC Â· destruir e restaurar", danger = true, callback = function()
	Lib.Destroy()
end })

--==============================================================================
-- 09 Â· HUD, INPUT, LOOPS E INIT
--==============================================================================

------------------------------------------------------------------- HUD ---------
local HUD = new("Frame", {
	Size = UDim2.fromOffset(168, 92), Position = UDim2.fromOffset(18, 60),
...
local hudLines = {}
for i = 1, 5 do
	hudLines[i] = new("TextLabel", {
		Size = UDim2.new(1, 0, 0, 15), BackgroundTransparency = 1, Font = Enum.Font.Code, TextSize = 11.5,
		TextColor3 = i == 1 and T.Accent or T.Sub, TextXAlignment = Enum.TextXAlignment.Left,
		Text = "", LayoutOrder = i, Parent = HUD,
	})
end
do
	...
	HUD.InputBegan:Connect(function(i)
		if i.UserInputType == Enum.UserInputType.MouseButton1 then
			dragging, startPos, startAbs = true, HUD.Position, i.Position
		end
	end)
	bind(UserInputService.InputChanged, function(i)
...
function Lib.Destroy()
	...
	pcall(applyLighting)
	pcall(function() lowGFX(false) end)
	pcall(function() noParticles(false) end)
	pcall(function() hideChar(false) end)
	local h = hum()
	...
	StarterGui:SetCore("SendNotification", { Title = "Nexus Admin", Text = "Encerrado e jogo restaurado.", Duration = 4 })
end
...
bind(UserInputService.InputBegan, function(input, processed)
	...
	elseif name == S.KB_ESP then
		S.ESP_Master = not S.ESP_Master
		if Lib.Flags.ESP_Master then Lib.Flags.ESP_Master(S.ESP_Master) end
	...
	elseif name == S.KB_Noclip then
		if Lib.Flags.Noclip then Lib.Flags.Noclip(not S.Noclip) end
...
bind(LocalPlayer.CharacterAdded, function(c)
	task.wait(0.6)
...
bind(RunService.RenderStepped, function(dt)
	...
	if not ok then
		espErrors += 1
		if espErrors % 120 == 1 then notify("Loop", tostring(err), 5, "warn") end
	end
end)
...
task.spawn(function()
	...
	while PanelGui.Parent do
		task.wait(0.5)
		local now = tick()
		local fps = math.floor(frames / math.max(now - last, 0.001))
		frames, last = 0, now
		local ping = 0
		...
		upLbl.Set(string.format("Uptime: %ds", math.floor(now - start)))
		if HUD.Visible then
			local r = root()
			hudLines[1].Text = fps .. " fps  " .. ping .. " ms"
			hudLines[2].Text = mem .. " mb"
			hudLines[3].Text = #Players:GetPlayers() .. " players  " .. #ESP.Targets .. " alvos"
			hudLines[4].Text = r and string.format("%d %d %d", r.Position.X, r.Position.Y, r.Position.Z) or "sem char"
			local h = hum()
			hudLines[5].Text = h and string.format("hp %d  spd %d", math.floor(h.Health), math.floor(h.WalkSpeed)) or ""
		end
	end
end)
...
notify("Pronto", "ESP na primeira aba. " .. S.KB_Panic .. " = encerrar tudo.", 6)
